# -*- coding: utf-8 -*
# TV-Over.NET python class
# (c) Eugene Bond, 2010-2012
# eugene.bond@gmail.com
# it's a modification of original plugin of Rodnoe.TV
...
try:											# Let's see if cookielib is available
	...
	xbmc.log('[TV-Over.NET] cookielib is not available..')
	pass
else:
...
try:
	...
	xbmc.log('[TV-Over.NET] module json is not available. using demjson')
	import demjson
...
class tvover:
	...
	def _request(self, cmd, params, inauth = None):
		...
		xbmc.log('[TV-Over.NET] Requesting %s' % url);
		...
		xbmc.log('[TV-Over.NET] Got %s' % rez, level=xbmc.LOGDEBUG)
		
		
		try:
			res = JSONDECODE(rez)
		except:
			xbmc.log('[TV-Over.NET] Error.. :(')
		...
		if COOKIEJAR != None:
			xbmc.log('[TV-Over.NET] Saving cookies: %s' % COOKIEFILE)
	...
	def initTimeFix(self):
		...
		if self.auto_timezone:
			xbmc.log('[TV-Over.NET] detecting local timezone')
			if time.timezone:
				self.time_zone = time.timezone / -60
			else:
				self.time_zone = 0
		xbmc.log('[TV-Over.NET] timezone set to %s' % self.time_zone)
		(val, opt) = self.getSettingCurrent('time_shift')
		self.timeshift = int(val)
		xbmc.log('[TV-Over.NET] time shift set to %s' % self.timeshift)
	...
	def getLast(self):
		...
		if not self.last_list:
			...
			try:
				...
				xbmc.log('[TV-Over.NET] Error loading last list %s' % e)
				res = self.getChannelsList()
			else:
				xbmc.log('[TV-Over.NET] last list loaded')
			f.close()
	...
	def getChannelsList(self):
		...
		try:
			...
			xbmc.log('[TV-Over.NET] Error saving last list %s' % e)
		else:
			xbmc.log('[TV-Over.NET] last list stored')
		f.close()
	...
	def getSettingsList(self):
		...
		xbmc.log('[TV-Over.NET] settings: %s' % res)
		return res
